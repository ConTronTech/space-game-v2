...
* **Anomalies** (`world::IAnomalies`, optional, docs/ANOMALIES.md): only DETECTED sites (Anomaly Scanner perk + in range + not investigated) as a pulsing violet 4-point star with a hollow diamond, same log range and the same height stem as bodies/stations (violet line, dimmer when below, base marker on the plane; `radarPlot`), drawn before stations; at most `kMaxRadarAnomalies` (16) looked at. Nothing extra without the perk.
