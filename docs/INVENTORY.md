...
the HUD / radar reads `hasPerk("ore_scanner")` to show the ore type and expected yield on a locked rock, ore-coloured radar dots and a tinted lock bracket (implemented: docs/MINING.md "Ore Scanner"). The set is pure (`gameplay::Perks` in `inventory_rules.h`).
