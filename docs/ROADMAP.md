...
- [x] **3.3 `world/planet_mesh`**: icosphere terrain, FBM noise, biomes, 4 LOD levels. *planet_mesh.h*
...
- [x] **3.7 `world/stations` + `ship/docking`**: orbital + planetary stations, dock/undock. *stations.h*  (**user instruction: keep docking SIMPLE - a cube and a cylinder as the platform placeholder until a proper model exists**; the old station OBJs in `assets/models/stations` can wait)
...
| **Station economy** (added 2026-09-21) | The user has specific plans for a station-based economy (trading/prices/contracts?) but the details are not shared yet, kept off the record for now. **Do not design or build station trading, prices, contracts or economy loops until the user's own spec arrives.** | Keep `world/stations` and `ship/docking` minimal and general (already true: a dock zone + a pad, nothing economy-specific). Leave the gap in Phase 6 rather than guessing at a design. |
