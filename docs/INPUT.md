...
`thrust strafe lift pitch yaw roll brake`, `toggle_warp` (Z), `toggle_orbit_lock` (O), **`fire` (left mouse), `weapon_1` / `weapon_2` / `weapon_3` (keys 1 / 2 / 3), `weapon_next` (mouse wheel), `lock_target` (T), `lock_clear` (Y)** (docs/COMBAT.md), **`toggle_menu` (I): the game menu (docs/GAME_MENU.md)**, `menu_discard` (Delete / Backspace): discard the selected CARGO grid slot, pressed twice (docs/INVENTORY.md), **`dock` (G)**: docks at / undocks from the nearest station (docs/STATIONS.md), `camera_next` (V), **`toggle_profiler` (F3), `toggle_profiler_gpu` (F4), `dump_profile` (F5)**: the live profiler (docs/PERFORMANCE.md), **`toggle_debugger` (F6), `toggle_debug_draws` (F7), `dump_debug_state` (F8)**: the state debugger (docs/DEBUGGER.md), `mouse_capture_toggle` (Tab), `pause` (Esc). The full list with keys is in docs/QUICKSTART.md.
