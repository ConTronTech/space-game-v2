...
**Marker:** a diamond around the target (at least 1.6 x its radius, and never smaller than ~2.5 % of its distance), camera-relative lines in the weapons pass' line batch: amber and blinking while acquiring, solid red with four inner ticks when locked; with the Ore Scanner perk (`IInventory::hasPerk("ore_scanner")`) both take the target's ore colour (data/ores.json, cached per ore). No extra draw call, no full-screen layer.
...
Missile additions (non-pure, for the HUD worker): `ammo(i)` (missiles left for a missile weapon, -1 = unlimited), `noAmmo()` ("NO MISSILES"), `missileCount()` (in flight), `lockState()` (`LockStateId` idle/acquiring/locked/lost), `lockTargetName()` ("asteroid 4092"; no ore since 5.5b), `lockTargetId()`, `lockTargetOre()` (raw ore id), `lockTargetRadius()` (the HUD decides what to show: `combat::lockInfoText` in `scanner_rules.h`, ore + yield only with the Ore Scanner perk), `lockTargetDistance()`, `lockTargetAngle()` (deg off the nose), `lockProgress()` (0..1).
