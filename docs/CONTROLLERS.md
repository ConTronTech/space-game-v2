...
User tuning (settings, applied live): `input.joystick_enabled` (true), `input.joystick_deadzone_scale` (0.5..2, multiplies every dead zone), `input.joystick_sensitivity` (0.1..4, multiplies stick / wheel axes, not pedals). The two multipliers are sliders on the Controllers screen's LIVE view.
...
`reloadProfiles()` re-reads `config/input/devices/*.json` and re-matches every open device without a restart (the Controllers screen calls it after SAVE).

## In-game setup: the Controllers screen (`ui/controller_setup`) - what players use
Pause menu > Settings > **Controllers**. It replaces the `joytest --guided` wizard for players (`joytest` stays as a standalone dev tool). The game is paused while it is open, so the controls under test never fly the ship.
* **DEVICES**: one row per connected device (name, `vid:pid`, axes / buttons / hats, the matched profile or "(generic: maps nothing)"); hot-plug updates live. Click one to select it.
* **LIVE**: the selected device: a bar per axis (flashes when it moves), a box per button (lit while down, press count), the hat as a cross; the joystick dead-zone and sensitivity sliders; and a **TEST** strip with the values the game really receives (`IInput::value` of yaw, thrust, brake, lift, roll, pitch: keyboard + mouse + profile), so a wrong sign or a stuck axis is obvious.
* **SETUP**: asks one control at a time: steering (turn fully right), throttle, brake, clutch (optional), then the buttons for fire, dock, warp, orbit lock, menu, camera, next weapon, previous weapon (optional), lock target (optional), lock clear (optional). It shows e.g. `GOT AXIS 6 (rests low, pressed +)`: **Enter**/OK confirms, **Backspace**/RETRY re-asks, BACK goes to the previous step, **Space**/SKIP skips an optional step. At the end a summary and **SAVE**.
  Detection is by CHANGE from a baseline taken at the start of each step (never by absolute value); the baseline follows the device for the first frames, a one-sample jump from an end to the centre is treated as a wrong first value being corrected, an axis must be HELD pressed / turned before OK, and a control already used by another step is ignored.
* **SAVE** writes `config/input/devices/<vid>_<pid>_<slug>.json` (header comments: generated by the in-game setup + date). Any existing profile for the same `vid:pid` is renamed to `.bak` first (never overwritten silently, and it would otherwise win the match), then `reloadProfiles()` makes it live.
  Generated values: steering `role steering`, dead zone 0.03, curve 1.5, `invert` true when the wheel reads + to the right (the game's `yaw` + = turn LEFT, so right must give yaw -); pedals `rest` min / max from where they are held pressed, dead zone 0.02 (and a `calibration` max/min when the pedal stops short of the end); clutch -> `lift` with `scale -1` (pressed = lift down); buttons -> `fire`, `dock`, `toggle_warp`, `toggle_orbit_lock`, `toggle_menu`, `camera_next`, `weapon_next`, `lock_target`, `lock_clear`.
  **Missing action:** the game has no `weapon_prev` yet: a button chosen for "previous weapon" is only noted in the header comment.
* Dev flags: `--controller-setup[=devices|live|setup]` opens it at start (screenshots); `--controller-setup=demo` runs the wizard with a simulated PXN V10 and saves at the end; `--controller-setup-out=DIR` writes the profile to DIR instead of `config/input/devices`.
* Code: `package/modules/ui/controller_setup/` (`controller_setup_rules.h` is pure: wizard state machine, profile generator, layout; tests in `package/tests/test_controller_setup.cpp`).

## Not done here
* No `input.joystick_enabled` toggle in the UI yet; the Controllers screen covers devices, live values, the two multipliers and the guided setup. `IControllers::lastEvent()` is reserved (always empty for now).
...
## The visual tester (`joytest`, dev tool: players use the Controllers screen)
