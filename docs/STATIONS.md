...
- **Planetary:** at a fixed latitude within +-60 degrees and a random longitude on the planet, **grounded on the local terrain**: `world::groundedRadius` samples the planet's terrain height (`world::surfaceRadiusFactor` in `planet_mesh.h`, the exact function the planet mesh vertices use: same seed `mixSeed(world.seed, body id)`, `world.terrain_height`, ocean flattening) at the centre, the 4 corners and 4 edge midpoints of the cube's footprint, and puts the cube's bottom face on the LOWEST of them, sunk 10% of the cube's height (`kStationEmbed`), so no gap shows on slopes (the uphill side is buried a little more). `up` stays the radial direction (not the slope normal), so the pad is level for docking. (Before this, the centre was at `1.02 R + 3.4 half`: on the default seed about 52 units above the local ground, on a pole.) **Planets do not rotate in V2**, so it never moves relative to its planet; it still travels with the planet around the sun (10-15 units/s).
...
Orbital: the WHOLE station spins about `up` (the orbit-plane normal) at `spinRate`. Planetary: the cube sits flat on the ground (no pedestal), pad on top, `up` = the radial direction, no spin. `info().position` is still the centre of the cube (unchanged meaning: the radar diamond and the dock zone use it).
