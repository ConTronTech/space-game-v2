...
// ship back) and a mining beam. Weapons are data (data/weapons.json). Rules in weapons_rules.h, design and tunables in docs/COMBAT.md.
#include <GL/gl.h>
#include <SDL2/SDL.h>
#include <algorithm>
#include <chrono>
...
class Weapons : public engine::Module, public combat::ICombat {
public:
    ...
    std::vector<std::string> optionalDependencies() const override {
        return {"ship/ship_core", "ship/fake_ship", "core/render_engine", "world/asteroids", "world/star_system", "world/stations", "fx/particles", "core/audio",
                "ship/docking", "ship/warp_drive", "core/data_registry", "core/camera"};
    }
    ...
    bool init(engine::Engine& eng) override {
        ...
        pool_.init(cap);
        lines_.assign((size_t)(cap + 8) * 2 * 3, 0); lineCol_.assign((size_t)(cap + 8) * 2 * 4, 0);
        heads_.assign((size_t)cap * 4 * 3, 0); headCol_.assign((size_t)cap * 4 * 4, 0);
        near_.reserve(512);
        ...
        active_ = true;
        LOG_I("combat", "%zu weapons, %d bolts max; selected: %s", weapons_.size(), cap, weapons_[sel_].name.c_str());
        return true;
    }
    ...
    void onFixedUpdate(engine::Engine& eng, float dt) override {
        ...
        steps_++;
        if (eng.time() - lastReport_ >= 1.0 && (firing_ || pool_.n > 0)) {
            lastReport_ = eng.time();
            LOG_D("combat", "%s heat %.2f%s, %d bolts, speed %.2f m/s, CPU per step %.4f ms", w.name.c_str(), hs.heat, hs.overheated ? " OVERHEATED" : "", pool_.n, ship->status().speed,
                  steps_ ? fireMs_ / steps_ : 0.0);
            fireMs_ = 0; steps_ = 0;
        }
    }
...
private:
    ...
    void draw(core::RenderEngine& r) {
        if (pool_.n == 0 && !beamActive_) return;
        float m[16];
